...
# --- Binaries under test ---
# Built from the pinned submodule, so it matches the enforcer library photon
# compiles against.
...
echo "Building photon_app..."
cargo build --manifest-path "$REPO_ROOT/Cargo.toml" --bin photon_app
...
PHOTON_APP='$REPO_ROOT/target/debug/photon_app'
EOF
