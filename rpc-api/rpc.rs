...
pub mod node {
    ...
    use l2l_openapi::open_api;
    use photon_types::{
        Address, Authorized, Block, BlockIndex, MainchainSyncProgress,
        MempoolTx, MerkleRoot, OutPoint, Output, OutputContent, Pointed,
        PointedOutput, SpentOutput, Transaction, Txid, WithdrawalBundle,
        net::Peer, schema as photon_schema,
    };

    use super::open_api;
    use crate::{GetTransactionResponse, schema};
    ...
    #[rpc(client, server, server_bounds(Self: open_api::RpcServer))]
    pub trait Rpc {
        /// Connect a block for which a BMM request was included in the
...
pub mod wallet {
    ...
    use photon_types::{
        ...
        PointedOutput, Transaction, Txid, schema as photon_schema,
        wallet::{Balance, TransferDests},
    };

    use super::open_api;
    use crate::{GetBlockTemplateResponse, schema};
