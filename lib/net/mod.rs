...
use crate::{
    ...
    types::{
        ...
        net::{
            DEFAULT_PORT, Peer, PeerConnectionStatus, ResolvedSeedAddress,
            SeedAddress,
        },
    },
    util::ErrorChain,
...
/// Network settings of a node
#[derive(Clone, Debug)]
pub struct Config {
    /// Peers to dial at each start. An IP address goes into the database.
    pub add_peers: HashSet<SeedAddress>,
    pub bind_addr: SocketAddr,
    pub magic_bytes_override: Option<peer_message::MagicBytes>,
    pub network: Network,
    /// Host names of the P2P server, in addition to `localhost`
...
mod test {
    ...
    use super::{ensure_seed_peers, resolve_seed_address, seed_node_addrs};
...
mod peer_handle_test {
    use super::{
        Config, DialSeedsHandle, Net, PeerConnectionCtxt, PeerConnectionInfo,
    ...
    fn temp_net_with_peers(
        ...
        let temp_dir =
            temp_dir::TempDir::with_prefix(format!("photon-{test_name}-"))?;
    ...
    -> anyhow::Result<()> {
        ...
        let addr = SocketAddr::from((Ipv6Addr::LOCALHOST, 4099));
        let resolved = ResolvedSeedAddress::Domain {
            domain: "localhost".to_owned(),
            port: addr.port(),
            addrs: nonempty::NonEmpty {
    ...
    async fn connect_peer_returns_other_quinn_errors() -> anyhow::Result<()> {
        ...
        net.server.close(0_u32.into(), b"test complete");
        let addr = SocketAddr::from((Ipv4Addr::LOCALHOST, 4099));
