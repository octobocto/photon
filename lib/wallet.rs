...
mod tests {
    ...
    fn test_get_receive_address() -> anyhow::Result<()> {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("photon_test_receive_{nanos}"));
        if test_dir.exists() {
    ...
    fn test_first_address_uses_index_zero() -> anyhow::Result<()> {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("photon_test_index0_{nanos}"));
        if test_dir.exists() {
            let _unused = std::fs::remove_dir_all(&test_dir);
        }
    ...
    fn test_legacy_wallet_adopts_index_zero() -> anyhow::Result<()> {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("photon_test_adopt0_{nanos}"));
        if test_dir.exists() {
            let _unused = std::fs::remove_dir_all(&test_dir);
        }
    ...
    fn funded_wallet(
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("photon_test_{name}_{nanos}"));
        if test_dir.exists() {
            let _unused = std::fs::remove_dir_all(&test_dir);
        }
        let wallet = Wallet::new(&test_dir)?;
